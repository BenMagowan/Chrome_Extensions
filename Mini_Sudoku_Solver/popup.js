/*
 * Popup logic for the Mini Sudoku Solver.
 *
...
const GAME_URL = "https://www.linkedin.com/games/mini-sudoku/";
...
const STATES = {
  checking: {
    ...
    hint: "Checking this tab for the Mini Sudoku game…",
    act: null,
  },
  idle: {
    status: "No board found",
    label: "Open Mini Sudoku game",
  ...
  solved: {
    ...
    status: (d) => {
      const base = `Solved · ${d.placed} digits placed`;
...
// How long the last solve took: counts up live while digits are placed, frozen
...
// The Mini Sudoku game page (guest and signed-in both live here). Subdomain
// optional so both `www.linkedin.com` and a bare `linkedin.com` match.
const GAME_URL_RE = /^https?:\/\/([a-z0-9-]+\.)*linkedin\.com\/games\/(view\/)?mini-sudoku/i;

// A tab's `url` is only populated for origins we hold host permission for, so a
// blank url already means "not LinkedIn" — and on the game page it's the
// mini-sudoku URL. Either way, a non-match means there's no puzzle to look for here.
function isGameUrl(url) {
  return typeof url === "string" && GAME_URL_RE.test(url);
}

/** Run runSudoku(mode, opts) in every frame; return the array of non-null results. */
...
async function refresh() {
  ...
  // If we're not on the Mini Sudoku game page, there is no board to find — go
  // straight to "open the game" instead of flashing "Looking for puzzle…" on an
  // unrelated tab, which is a claim that only makes sense on the game itself.
  const tab = await getActiveTab();
