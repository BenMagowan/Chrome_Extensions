 ...
 * it shares the page's DOM, so it drives the same `runZip` engine directly.
 * `runZip` is defined by injected.js, which the manifest lists BEFORE this file
...
(async () => {
  ...
  for (let i = 0; i < TRIES; i++) {
    let res = null;
    try {
      res = await runZip("detect"); // from injected.js (same content-script scope)
    } catch {
    ...
    if (res && res.solvable) {
      ...
      try {
        const r = await runZip("solve", { targetMs });
        console.log(TAG, r && r.ok ? `done — ${r.placed}-cell path drawn` : "solve did not complete", r);
      } catch (e) {
