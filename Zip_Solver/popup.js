...
const STATES = {
  checking: {
    ...
    hint: "Checking this tab for the Zip game…",
    act: null,
  },
  idle: {
  ...
  solved: {
    ...
    status: (d) => {
      const base = `Solved · ${d.placed}-cell path drawn`;
 ...
 * this the four square corner cells are clipped and their hairlines cut off at
 * the curve. Works for any N since it keys off row/col.
 */
...
function buildGrid(N) {
  ...
  for (let i = 0; i < N * N; i++) {
    ...
    const corner = cornerClass(Math.floor(i / N), i % N, N);
...
// How long the last solve took: counts up live while the path is drawn, frozen
...
// The Zip game page (guest and signed-in both live here). Subdomain optional
// so both `www.linkedin.com` and a bare `linkedin.com` match.
const GAME_URL_RE = /^https?:\/\/([a-z0-9-]+\.)*linkedin\.com\/games\/(view\/)?zip/i;

// A tab's `url` is only populated for origins we hold host permission for, so a
// blank url already means "not LinkedIn" — and on the game page it's the zip
// URL. Either way, a non-match means there's no puzzle to look for here.
function isGameUrl(url) {
  return typeof url === "string" && GAME_URL_RE.test(url);
}

/** Run runZip(mode, opts) in every frame; return the array of non-null results. */
...
async function refresh() {
  ...
  // If we're not on the Zip game page, there is no board to find — go straight
