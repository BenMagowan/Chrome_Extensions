/*
 * Popup logic for the Zip Auto-Solver.
 *
...
const ZIP_URL = "https://www.linkedin.com/games/zip/";
...
async function refresh() {
  ...
  if (board) {
    ...
    mode = "open";
    solveBtn.disabled = false;
    solveBtn.textContent = "Open Zip game";
    statusEl.textContent = "No board detected. Open the Zip game to get started.";
  }
}

solveBtn.addEventListener("click", async () => {
  if (mode === "open") {
    // No board is present — open the Zip game in a new tab.
  ...
  stopPolling();
  statusEl.textContent = "Solving… (drawing the path)";
  const results = await runInFrames("solve");
  const ok = results.find((r) => r.ok);
  if (ok) {
    statusEl.textContent = `Done — drew a ${ok.placed}-cell path. ⚡`;
  } else {
