 ...
 * it shares the page's DOM, so it drives the same `runTango` engine directly.
 * `runTango` is defined by injected.js, which the manifest lists BEFORE this file
...
(async () => {
  ...
  for (let i = 0; i < TRIES; i++) {
    let res = null;
    try {
      res = await runTango("detect"); // from injected.js (same content-script scope)
    } catch {
    ...
    if (res && res.solvable) {
      ...
      try {
        const r = await runTango("solve", { targetMs });
        console.log(TAG, r && r.ok ? `done — ${r.placed} symbols placed` : "solve did not complete", r);
      } catch (e) {
