...
const STATES = {
  checking: {
    ...
    hint: "Checking this tab for the Queens game…",
    act: null,
  },
  idle: {
  ...
  solved: {
    ...
    // doesn't advertise a step that didn't happen. The elapsed time is appended
    // here too (not just on the visible chip) so the live region reads it aloud.
    status: (d) => {
      const base = d.cleared
 ...
 * this the four square corner cells are clipped and their colour reads as "cut
 * off" (a dark notch). Works for any N since it keys off row/col.
 */
...
// Two settings: Auto-solve (off by default — opt-in, so nothing touches the board
// unasked) and the target Solve time in whole seconds.
//
...
//    closes right after a toggle can be torn down before that write flushes — which
//    is exactly why the settings weren't sticking.
...
function mirrorToWorker() {
  try {
    chrome.storage.local.set({ [AUTOSOLVE_KEY]: autoSolve, [TARGET_KEY]: targetMs });
  } catch {
    /* worker mirror unavailable — the popup's own localStorage copy still holds */
  }
}
...
// How long the last solve took: counts up live while crowns are placed, frozen
...
// The Queens game page (guest and signed-in both live here). Subdomain optional
// so both `www.linkedin.com` and a bare `linkedin.com` match.
const GAME_URL_RE = /^https?:\/\/([a-z0-9-]+\.)*linkedin\.com\/games\/queens/i;

// A tab's `url` is only populated for origins we hold host permission for, so a
// blank url already means "not LinkedIn" — and on the game page it's the queens
// URL. Either way, a non-match means there's no puzzle to look for here.
function isGameUrl(url) {
  return typeof url === "string" && GAME_URL_RE.test(url);
}

/** Run runQueens(mode, opts) in every frame; return the array of non-null results. */
...
/**
 * Solve the current board. Shared by the Solve button and the auto-solve path,
...
function init() {
  ...
  renderPlaceholder();
  setState("checking");
  // Load settings (synchronous — see loadSettings) before the first detect so
  // auto-solve can fire on it rather than only catching the next poll.
  loadSettings();
  refresh();
