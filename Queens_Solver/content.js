 ...
 *
 * This is what makes "Auto-solve" run WITHOUT the popup being opened. It replaces
 * the old background service worker, which was unreliable: an MV3 worker sleeps,
 * and whether tabs.onUpdated actually woke it (and handed it the URL) was exactly
 * the kind of thing that failed silently — the symptom being "auto-solve only
 * works once you click the popup".
 *
 ...
 * it shares the page's DOM, so it drives the same `runQueens` engine directly.
 * `runQueens` is defined by injected.js, which the manifest lists BEFORE this file
...
(async () => {
  ...
  for (let i = 0; i < TRIES; i++) {
    let res = null;
    try {
      res = await runQueens("detect"); // from injected.js (same content-script scope)
    } catch {
    ...
    if (res && res.solvable) {
      ...
      console.log(TAG, `solving ${res.N}×${res.N} board (target ${targetMs == null ? "default" : targetMs + "ms"})`);
      try {
        const r = await runQueens("solve", { targetMs });
        console.log(TAG, r && r.ok ? `done — ${r.placed} crowns placed` : "solve did not complete", r);
      } catch (e) {
